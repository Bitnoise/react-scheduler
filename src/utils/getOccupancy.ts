...
export const getOccupancy = (
  ...
  if (resourceIndex < 0) return { taken: 0, free: 0, overtime: 0 };
  const occupancy = resource.flat(2).filter((item) => {
    if (zoom === 0) {
      ...
      return startWeek <= focusedWeek && endWeek >= focusedWeek;
    } else {
      const startDay = dayjs(item.startDate).dayOfYear();
      const endWeek = dayjs(item.endDate).dayOfYear();
      return startDay <= focusedDayNum && endWeek >= focusedDayNum;
    }
  });
